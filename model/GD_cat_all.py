...
class SECatBottleneckX(nn.Module):
    ...
    def __init__(self, inplanes, planes, cat_channel, cardinality=16, stride=1, downsample=None):
        ...
        self.conv1 = nn.Conv2d(inplanes + cat_channel, planes, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
        self.conv2 = nn.Conv2d(planes + cat_channel, planes, kernel_size=3, stride=stride,
                               padding=1, groups=cardinality, bias=False)
        self.bn2 = nn.BatchNorm2d(planes)

        self.conv3 = nn.Conv2d(planes + cat_channel, planes * self.expansion, kernel_size=1, bias=False)
